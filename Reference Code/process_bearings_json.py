...
def process_bearing_folder(bearing_folder, bronze_path, data_root_path):
    ...
    for filename in os.listdir(folder_path):
        if filename.startswith('acc') and filename.endswith('.csv'):
            ...
            # TODO: MAKE THIS DYNAMIC TO THE NUMBER OF CHANNELS
            channel_1 = df.iloc[:, -2].tolist()
            channel_2 = df.iloc[:, -1].tolist()

            # Create timestamp column
            # TODO: THIS MAY NEED MODIFICATION BASED ON THE TIMESTAMP FORMAT
            df['timestamp'] = pd.to_datetime(
                ...
                format='%H:%M:%S',
                errors='coerce'
            ).dt.time # Only keep the time part TODO: THIS MAY NEED MODIFICATION
            ...
            combined_data_entry = {
                ...
                'timepoint_index': int(filename.split('.')[0].split('_')[-1]),
